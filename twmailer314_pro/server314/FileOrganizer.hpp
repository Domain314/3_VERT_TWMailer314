...
private:
    ...
    bool checkForIpBan(string* userIp);
    bool extractIPs(fs::path* path, string* userIp);
    void addIpToBlockList(string* userIp);
    bool checkIpTime(string* line, int divider, int timestamp);
    bool checkIpAddr(string* line, int divider, string* userIp);
    bool tryToLogin(string* userName, string* userPassword);
    bool signupUser(string* userName, std::string *userPassword);

    void pushList(string* command, string* answer);
